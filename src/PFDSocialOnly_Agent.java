...
public class PFDSocialOnly_Agent extends Agent {
    PFDSocialOnly_Agent(MailManager[] mailManager, int AgentNo, Vector<Integer> neighbors, int parent, Vector<Integer> child, int population, double domain_lb, double domain_ub, int maxIteration, boolean ismax, ArrayList<Edge> edgelist, int[][] indexToEdge, HashMap<Edge, Constraint> constraints, double w, double c1, double c2) {
    ...
    public void updateValues()
    {
        ...
        for(int i = 0; i < population; i++)
        {
            ...
            if(i==this.gbestParticleNo)
            {
                ...
                this.velocity[i] = w * this.velocity[i] + c2 * r2 * (this.gbest_position - this.position[i]);
                this.position[i] = this.position[i] + this.velocity[i];
